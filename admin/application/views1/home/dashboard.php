                        ...
                        <tbody>
                           ...
                           <?php foreach ($orders as $a){ $secret = md5($a->orderID);
                              ...
                              $user = $this->db->get_where('users',array('ID'=>$a->userID))->row();
                                    $amt =   $a->total_amount;
                                  $cash_amt = $a->cashback_amount;
                                  $dis_amt = $a->coupon_discount;
                                  $delivery_charges = $a->delivery_charges;

                              if($a->type == 'CASHBACK'){ 
                                   $payable_amt = $a->total_amount;
                                }else{
                                 $payable_amt = $a->total_amount;
                              } ?>
                           ...
                           <tr>
                              ...
                              <td><?php if($a->type=='CASHBACK'){ ?>
                                    <p><strong>CASHBACK  : </strong>₹ <?=$a->cashback_amount?></p>
                               <?php }elseif($a->type=='FIXED'){?>
                                 <p><strong>Discount  : </strong>₹ <?=$a->coupon_discount?></p>
                              <?php }elseif($a->type=='PERCENTAGE'){?>
                                  <p><strong>Discount  : </strong>₹ <?=$a->coupon_discount?></p>
                              <?php }?>
                              </td>
                              <td><?=$payable_amt?></td>
