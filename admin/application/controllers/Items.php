...
class Items extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if (isset($_GET['cat'])) {
            ...
            if (empty($sub_cat)) {
                // Handle the case when sub-categories are not found
    ...
    public function add()
    {
        ...
        if ($_POST) {
            ...
            $property_type_id   = trim($_POST['property_type'] ?? '');
            ...
            // $this->data['category'] = $this->home_m->get_all_row_where('category', array('parent' => 0), $select = 'categoryID,title');
            $this->data['item_category'] = $this->home_m->get_all_row_where('item_category', array('parent' => 0, 'status' => 'Y'), $select = 'categoryID,title');
            $this->data['society'] = $this->home_m->get_all_row_where('products', array('status' => 'active'), $select = '*');
            $this->data['sub_view'] = 'items/add';
            $this->data['title'] = 'Add Product';
            $this->load->view("_layout", $this->data);
    ...
    public function get_floor_type()
    {
        $this->load->database();
        $society_id = $this->input->post('society_id');

        $query = $this->db->query(
            "SELECT floor_id, floor_type FROM floor_type WHERE property_id = ? AND status = 'active'",
            array($society_id)
        );
    ...
    public function get_floor_type_feature()
    {
        $this->load->database();

        $society_id = $this->input->post('society_id');
        $property_type_ids = $this->input->post('property_type_id'); // array expected

        if (!is_array($property_type_ids)) {
            $property_type_ids = explode(',', $property_type_ids);
        }

        $this->db->where('property_id', $society_id);
        $this->db->where_in('property_type_id', $property_type_ids);
        $this->db->where('status', 'active');
        $query = $this->db->get('floor_dimensions');
    ...
    // Custom callback function to check if selling price is greater than price
    public function check_selling_price($selling_price)
    {
        $price = $this->input->post('price');
        if ($selling_price > $price) {
            // Validation failed, add an error message
            $this->form_validation->set_message('check_selling_price', 'The Selling Price must be less than or equal to the Price.');
            return FALSE;
        }
        return TRUE;
    }
    ...
    public function add_variants()
    {
        if ($_POST) {
            ...
            $insert_array['cost_price'] = $this->input->post('cost_price');
            $insert_array['weight'] = $this->input->post('unit_value');
            if (!empty($_FILES['product_image']['name'])) {
                $target_path = 'uploads/variants/';
                $extension = substr(strrchr($_FILES['product_image']['name'], '.'), 1);
                $actual_image_name = 'product' . time() . "." . $extension;
            ...
            $this->city_wise_variant_price($product_id, $insert_array['retail_price'], $this->input->post('price'), $this->input->post('unit_value'), $this->input->post('unit'), $this->input->post('stock_count'), $this->input->post('cost_price'), $is_default = 0, $this->input->post('in_stock'), $actual_image_name, $vegtype, $insert_array['weight']);
            //echo $this->db->last_query(); exit;
            redirect(base_url("products"));
        } else {
            //$this->data['category'] = $this->home_m->get_all_row_where('category',array('parent'=>0),$select='categoryID,title');
    ...
    public function update_singleVarinat1()
    {
        ...
        foreach ($data_Variant['retail_price'] as $key => $data) {
            $info = array(
                'retail_price' => isset($data_Variant['retail_price'][$key]) ? $data_Variant['retail_price'][$key] : '',
