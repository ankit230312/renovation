...

    {
...

        {
...

            {
                ...
                $this->db->query("UPDATE `users` SET `referral_userID`='$referral_userID' WHERE `ID`='$userID'");
...

{
    ...
    $response[] = array('result'=>'success','share_earn'=>'Hey user, Please use my referral code to register to gowisekart and get cashback on first order. Use My Code is '.$user_info->referral_code.'. and to Download latest gowisekart App from play store using https://play.google.com/store/apps/details?id=com.gowisekart .', 'gift_banner'=>$gift_banner, 'new_banner'=>$new_banner, 'new_banner1'=>$new_banner1);
...

{
    ...
    $products = $this->webservice_m->get_all_table_query("SELECT * FROM `products` WHERE productID IN (SELECT product_id FROM products_variant WHERE city_id = '$cityID' AND `in_stock` = 'Y') AND `in_stock` = 'Y' AND `product_name` LIKE '%$key%' OR `tags` LIKE '%$key%'");
...

{
    ...
    if(!empty($user)){
        if(!empty($key) && $key!=''){
            $products = $this->webservice_m->get_all_table_query("SELECT `productID`,`product_name`,`product_image` FROM `products` WHERE productID IN (SELECT product_id FROM products_variant WHERE city_id = '$user->cityID' AND `status` = 'Y') AND `in_stock` = 'Y' AND `product_name` LIKE '%$key%' OR `tags` LIKE '%$key%'");
            if(!empty($products)){
...
public function transaction_history(){
    ...
    if(!empty($userID)){
        ...
        $response[] = array('result'=>'success', 'transactions'=>$transactions);
...

{
   ...
   /* echo $this->db->last_query();
    ...
    $a = array('userID'=>$userID,'productID'=>$productID, 'variantID'=>$variantID,'qty'=>$qty,'added_on'=>date('Y-m-d H:i:s'));
...

{
    ...
    if (!empty($user_info)){
        ...
        $result[] = array("result"=>"success","message"=>"Successfully Retrieved","profile"=>$user_info,'refer_text'=>'Hey user, Please use my referral code to register to gowisekart and get cashback on first order. Use My Code is '.$user_info->referral_code.'. and to Download latest gowisekart App from play store using https://play.google.com/store/apps/details?id=com.gowisekart .');
    ...
    if(!empty($totalOrder)){
        ...
        if(count($totalOrder) == 1){
            ...
            if(!empty($user->referral_userID)){
              ...
              $this->db->update('users',array('wallet'=>$referalUser->wallet+10));
